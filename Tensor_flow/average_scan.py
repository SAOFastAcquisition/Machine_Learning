...
if __name__ == '__main__':
    path_stocks = '2023-11-09_01+24_stocks.npy'
    ...
    for i in range(50):
        ...
        # plt.plot(np.array(freq[125:181]) / 3.e4, a, label=f't = {np.ceil(time[t0 + i * dt])} s') [125:181]
        plt.plot(np.array(freq), a, label=f't = {np.ceil(time[t0 + i * dt])} s')

    plt.grid('both')
    plt.title(path_stocks[:-4])
    plt.ylabel('Normalized intensity')
    # plt.xlabel('Wavenumber, cm_-1') [:, 125:181]
    ...
    graph_contour_2d(freq, time, data_norm1, s, _info_txt,
                     'Normalized Antenna Temperature ' + path_stocks[: -4], _head)
