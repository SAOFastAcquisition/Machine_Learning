...
def f_add(_x_batch, _level_mod, _edge_mod_l, _edge_mod_r, _width_mod_l, _width_mod_r):
    a = tf.Variable(tf.zeros_like(_x_batch, dtype=float))
    return a
...
def model():
    ...
    _data = sun_with_noise(tf.Variable(_x_n, dtype=float), 1.0, -19.0, 21.0, 2.0, 2.5) + \
...
def sun_optimiser(_x_n, _data, _EPOCHS, _BATCH_SIZE, _learn_rate, _flag):
    _opt = tf.optimizers.Adam(learning_rate=_learn_rate)  # EPOCHS = 80

    _loss_dyn = np.array([])

    _TOTAL_POINTS = len(x_n)
    _num_steps = _TOTAL_POINTS // _BATCH_SIZE

    for n in range(_EPOCHS):
        for _n_batch in range(_num_steps):
            _y_batch = _data[_n_batch * _BATCH_SIZE: (_n_batch + 1) * _BATCH_SIZE]
            _x_batch = _x_n[_n_batch * _BATCH_SIZE: (_n_batch + 1) * _BATCH_SIZE]

            with tf.GradientTape() as _t:
                _f = flare(_x_batch, height_mod1, pos_mod1, width1_mod1) + b + \
                     f_add(_x_batch, level_mod, edge_mod_l, edge_mod_r, width_mod_l, width_mod_r)
                if _flag == 1:
                    _f = quiet_sun(_x_batch, level_mod, edge_mod_l, edge_mod_r, width_mod_l, width_mod_r) + \
                         flare(_x_batch, height_mod1, pos_mod1, width1_mod1) + b
                _d_loss = tf.reduce_mean(tf.square(_y_batch - _f))

            _dp1, _dp2, _dp3, _dp4, _dp5, _db, _dh, _dp, _dw = _t.gradient(_d_loss,
                                                                           [level_mod, edge_mod_l, edge_mod_r,
                                                                            width_mod_l,
                                                                            width_mod_r, b,
                                                                            height_mod1, pos_mod1, width1_mod1])
            _opt.apply_gradients(zip([_dp1, _dp2, _dp3, _dp4, _dp5, _db, _dh, _dp, _dw],
                                     [level_mod, edge_mod_l, edge_mod_r, width_mod_l, width_mod_r, b,
                                      height_mod1, pos_mod1, width1_mod1]))
            _loss_dyn = np.append(_loss_dyn, _d_loss.numpy())
        _n_batch = 0

    plt.plot(_loss_dyn)
    plt.show()

    return level_mod, edge_mod_l, edge_mod_r, width_mod_l, width_mod_r, b, height_mod1, pos_mod1, width1_mod1
...
#     n_batch = 0
sun_calc1 = flare(x_n, height_mod1, pos_mod1, width1_mod1) + b

dat2 = dat - sun_calc1.numpy()

print(b, height_mod1, pos_mod1, width1_mod1, sep='\n')
